#include "rfcn_multiclass_split_predict.h"
#include <algorithm>
...
void RFCN_detector::forward(vector<cv::Mat> imgs, vector<boost::shared_ptr<Blob<float>>>& person_upper_outputs, vector<boost::shared_ptr<Blob<float>>>& person_face_outputs) {
  Blob<float> *input_layer = net_->input_blobs()[0];
  ...
  boost::shared_ptr<Blob<float>> person_upper_rois =
      net_->blob_by_name(string("person_upper_rois"));
  boost::shared_ptr<Blob<float>> person_upper_cls_prob =
      net_->blob_by_name(string("person_upper_cls_prob"));
  boost::shared_ptr<Blob<float>> person_upper_bbox_pred =
      net_->blob_by_name(string("person_upper_bbox_pred"));
  person_upper_outputs.push_back(person_upper_rois);
  person_upper_outputs.push_back(person_upper_cls_prob);
  person_upper_outputs.push_back(person_upper_bbox_pred);

  boost::shared_ptr<Blob<float>> person_face_rois =
      net_->blob_by_name(string("person_face_rois"));
  boost::shared_ptr<Blob<float>> person_face_cls_prob =
      net_->blob_by_name(string("person_face_cls_prob"));
  boost::shared_ptr<Blob<float>> person_face_bbox_pred =
      net_->blob_by_name(string("person_face_bbox_pred"));
  person_face_outputs.push_back(person_face_rois);
  person_face_outputs.push_back(person_face_cls_prob);
  person_face_outputs.push_back(person_face_bbox_pred);
}
...
void DetectionForVideo(string &model_file, string &trained_file, YAML::Node &config, string &img_file, bool x_show) {
  ...
  parse_vector<float>(config["TEST"]["THRESHS"], score_thres_vec);
  ...
  while (infile >> img_path >> anno_path) {
    ...
    vector<boost::shared_ptr<Blob<float>>> person_upper_outputs;
    vector<boost::shared_ptr<Blob<float>>> person_face_outputs;
    gettimeofday(&start, NULL);
    rfcn_det.forward(images, person_upper_outputs, person_face_outputs);
    gettimeofday(&end, NULL);
	cout<<"forward time: "<< double(end.tv_sec - start.tv_sec)*1000.0 + double(end.tv_usec - start.tv_usec) / 1.0e3 <<endl;                                                                         

    vector<struct Bbox> person_upper_result;
    vector<struct Bbox> person_face_result;
    gettimeofday(&start, NULL);
	rfcn_det.get_detection(person_upper_outputs, person_upper_result, score_thres_vec[0], nms_thres, resize_height, resize_width, enlarge_ratio);
	rfcn_det.get_detection(person_face_outputs, person_face_result, score_thres_vec[1], nms_thres, resize_height, resize_width, enlarge_ratio);
    gettimeofday(&end, NULL);
    ...
    if (x_show) {
      ...
      for (int bbox_id = 0; bbox_id < person_upper_result.size(); bbox_id++) {
        rectangle(frame, person_upper_result[bbox_id].rect, Scalar(0, 255, 0),
                  1.0);
        sprintf(str_info, "%.3f", person_upper_result[bbox_id].confidence);
        string prob_info(str_info);
        putText(frame, prob_info, Point(person_upper_result[bbox_id].rect.x, person_upper_result[bbox_id].rect.y),
                CV_FONT_HERSHEY_SIMPLEX, 0.3, Scalar(0, 0, 255));
      }

      for (int bbox_id = 0; bbox_id < person_face_result.size(); bbox_id++) {
        rectangle(frame, person_face_result[bbox_id].rect, Scalar(0, 0, 255),
                  1.0);
        sprintf(str_info, "%.3f", person_face_result[bbox_id].confidence);
        string prob_info(str_info);
        putText(frame, prob_info, Point(person_face_result[bbox_id].rect.x, person_face_result[bbox_id].rect.y),
                CV_FONT_HERSHEY_SIMPLEX, 0.3, Scalar(0, 0, 255));
      }
