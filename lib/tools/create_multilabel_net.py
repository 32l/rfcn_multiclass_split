...
def setupDataLayers(net_type):
	...
	if net_type == "train":
		exec ('''net['data'], net['im_info'], {} = L.Data(name="input-data", type="Python",
															 ...
															 ntop={})'''.format(multilabel_gt_box_str,
																				cfg.NUM_CLASSES + 1))
	if net_type == "deploy":
...
def add_train_rfcn_layers(net, split_to_rpn_layer, end_body_layer=None, prefix=""):
	...
	num_classes = 2
	class_name = prefix[:-1]
	assert class_name in cfg.CLASSES
	anchor_generate = cfg.ANCHOR_GENERATOR[class_name]
	if anchor_generate.POLICY == "SCALE_RATIO":
		anchors_num = len(anchor_generate.SCALES) * \
					  len(anchor_generate.RATIOS)
	if anchor_generate.POLICY == "SLIDING_WINDOW":
		anchors_num = len(anchor_generate.SLIDING_WINDOW_WIDTH)
	kwargs = {
		'param': [dict(lr_mult=1, decay_mult=1), dict(lr_mult=2, decay_mult=0)],
		'weight_filler': dict(type='msra'),
		'bias_filler': dict(type='constant', value=0.0)}
	proposal_param = {'ratio': anchor_generate.RATIOS, 'scale': anchor_generate.SCALES,
	...
	net[prefix + 'rpn_labels'], net[prefix + 'rpn_bbox_targets'], net[prefix + 'rpn_bbox_inside_weights'], net[
		...
		net[prefix + 'rpn_cls_score'], net[prefix + 'gt_boxes'], net['im_info'], net['data'], name=prefix + "rpn-data",
		python_param={'module': "rpn.anchor_target_layer", 'layer': "AnchorTargetLayer",
					  'param_str': str(dict(class_name=class_name))}, ntop=4)
	...
	# python_param={'module': "rpn.proposal_layer", 'layer': "ProposalLayer",'param_str': str(dict(class_name = class_name))},
	# ntop=2)
	...
	net[prefix + 'loss_cls'] = L.SoftmaxWithLoss(net[prefix + 'cls_score'], net[prefix + 'labels_ohem'],
												 name=prefix + "loss",
												 loss_weight=1.0,
...
def add_deploy_rfcn_layers(net, split_to_rpn_layer, end_body_layer=None, prefix=""):
	...
	net[prefix + 'bbox_pred_pred'] = L.Pooling(net[prefix + 'psroipooled_loc_rois'], name=prefix + "ave_bbox_pred_rois",
											   pool=P.Pooling.AVE,
											   kernel_size=position_num, stride=position_num)
	net[prefix + 'bbox_pred'] = L.Reshape(net[prefix + 'bbox_pred_pred'], name=prefix + "bbox_pred_reshape",
										  reshape_param={
											  'shape': {'dim': [-1, 8]}})
	return net
