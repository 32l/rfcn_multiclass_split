...
class AnchorTargetLayer(caffe.Layer):
	def setup(self, bottom, top):
		self.class_name = eval(self.param_str)["class_name"]
		anchor_generator = cfg.ANCHOR_GENERATOR[self.class_name]
	...
	for batch_ind in range(img_blob_batch.shape[0]):
		...
		for i in range(len(inds_1)):
			...
			cv2.rectangle(img, (gt_box[0], gt_box[1]), (gt_box[2], gt_box[3]), color,
						  1)
		# cv2.putText(img, "gt", (gt_box[0], gt_box[1]), cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 1)
