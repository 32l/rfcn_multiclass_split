...
def hyy_cfg_from_file(filename):
	with open(filename, 'r') as f:
		cfg = edict(yaml.load(f))
	return cfg

new_cfg= hyy_cfg_from_file("config.yaml")
...
__C.TEST.THRESHS = []
__C.TEST.THRESH = 0.01


# anchor generator
__C.ANCHOR_GENERATOR = edict()
for class_name in new_cfg.CLASSES:
	__C.ANCHOR_GENERATOR[class_name]=edict()
	__C.ANCHOR_GENERATOR[class_name].POLICY = "SCALE_RATIO"
	# __C.ANCHOR_GENERATOR[class_name].SCALES = (32, 64, 128)
	__C.ANCHOR_GENERATOR[class_name].SCALES = (128, 256, 512)
	__C.ANCHOR_GENERATOR[class_name].RATIOS = (0.5, 1, 2)
	__C.ANCHOR_GENERATOR[class_name].SLIDING_WINDOW_WIDTH = (32,)
	__C.ANCHOR_GENERATOR[class_name].SLIDING_WINDOW_HEIGHT = (32,)
