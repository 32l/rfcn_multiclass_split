...
def get_minibatch(roidb):
	img_blob, gt_boxes_blob_raw, im_info = preprocess(roidb)
	if gt_boxes_blob_raw.size == 0:
		return None
	if cfg.MULTICLASS_SPLIT:
		gt_boxes_blob = []
		for i in range(1, cfg.NUM_CLASSES):
			cls_gt_boxes_blob = gt_boxes_blob_raw[gt_boxes_blob_raw[:, 4] == i, :]
			cls_gt_boxes_blob[:, 4] = 1
			if cls_gt_boxes_blob == None:
				return None
			else:
				gt_boxes_blob.append(cls_gt_boxes_blob)
	else:
		gt_boxes_blob = gt_boxes_blob_raw
...
def convert_gt_boxes(gt_boxes, crop_rect):
	...
	if len(base_inds) == 0 or base_inds.size == 0:
		...
		inds = gt_boxes_area[base_inds] * 1.0 / ori_gt_boxes_area[base_inds] > 0.7
