...
class RoIDataLayer(caffe.Layer):
	...
	def forward(self, bottom, top):
		...
		if cfg.MULTICLASS_SPLIT:
			for i in range(0, cfg.NUM_CLASSES - 1):
				top[2 + i].reshape(*(gt_boxes_blob[i].shape))
				top[2 + i].data[...] = gt_boxes_blob[i].astype(np.float32, copy=False)
		else:
	...
	def vis(self, im_blob, gt_boxes_blob):
		...
		import cv2
		import sys
		colors=[(0,0,255),(0,255, 0), (0,0,255), (0,255,255), (255,0,255),(255,255,0)]
		for i in xrange(im_blob.shape[0]):
			...
			if cfg.MULTICLASS_SPLIT:
				for cls_ind, gt_boxes_blob_ in enumerate(gt_boxes_blob):
					for bbox_info in gt_boxes_blob_:
						batch_ind = bbox_info[-1]
						if batch_ind != i:
							continue
						cls = bbox_info[-2]
						bbox = bbox_info[:4]
						cv2.rectangle(im, (bbox[0], bbox[1]), (bbox[2], bbox[3]), colors[cls_ind], 1)
						cv2.putText(im, str(int(cls)), (bbox[0], bbox[1]), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
					cv2.imshow("debug", im)
					key = cv2.waitKey()
					if key == -1 or key == 27:
						sys.exit(0)
				...
				for bbox_info in gt_boxes_blob:
					...
					cls = bbox_info[-2]
					bbox = bbox_info[:4]
					cv2.rectangle(im, (bbox[0], bbox[1]), (bbox[2], bbox[3]), colors[int(cls)-1], 1)
					cv2.putText(im, str(int(cls)), (bbox[0], bbox[1]), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 0, 255), 1)
				cv2.imshow("debug", im)
				key = cv2.waitKey()
				if key == -1 or key == 27:
